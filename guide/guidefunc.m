...
try
    ...
    switch action,
        ...
        case 'helpGettingStarted'
             helpview([docroot '/techdoc/creating_guis/creating_guis.map'], 'guide_getting_started');
            
        case 'helpLayingOutGUIs'
            helpview([docroot '/techdoc/creating_guis/creating_guis.map'], 'layout_editor');

        case 'helpProgrammingGUIs'
             helpview([docroot '/techdoc/creating_guis/creating_guis.map'], 'guide_programming');
            
         case 'helpExampleGUIs'
             helpview([docroot '/techdoc/creating_guis/creating_guis.map'], 'application_techniques');
...
for i=1:length(objs)
    children =[];
    if isa(handle(objs{i}),'uicontainer')
...
% ****************************************************************************
% Delete HG obejct(s) without triggering the DeleteFcn
% ****************************************************************************
...
if ~isExternalControl(obj)
    ...
    % keep that informaion in another way
...
if (flag>0)
    ...
    choiceTitle = xlate('To continue activation');
    dialog = PathUpdateDialog(frame, 'GUIDE', filename, choiceTitle, mdbFlag);
...
[p, f, e, ErrMsg] = checkValidFilename(filename, '.fig', false);
...
if ~isempty(ErrMsg)
    edtMethod('showMessageDialog', 'com.mathworks.mwswing.MJOptionPane', ...
        frame, ErrMsg, 'GUIDE', MJOptionPane.ERROR_MESSAGE);
    return;
end
...
if isoverwrite
    if isappdata(0, 'templateFile')
        ...
        % mode, it is now. For template nosave mode, it is the time when
        ...
        % change filename to empty. For template nosave, this will prevent
        % it from being added to MRU list. For tempalte save, this will be
        % reset later
        set(fig,'FileName','');
    ...
    if match
        ...
        edtMethod('showMessageDialog', 'com.mathworks.mwswing.MJOptionPane', ...
            frame, [prompt, 10, target, 10, message], 'GUIDE', MJOptionPane.ERROR_MESSAGE);
    end
end
if ~needsave
    return;
...
if found
    ...
    if ~status.mfile.exist && ~status.figure.exist
        ErrMsg = [prompt, sprintf('\n'), flist, 'are invalid filenames'];
        ErrMsg = sprintf('%s\n\nPlease change the "Filename" property in the inspector to a\nvalid value, or use the Save As option.', ErrMsg);
        edtMethod('showMessageDialog', 'com.mathworks.mwswing.MJOptionPane', ...
            frame, ErrMsg, 'GUIDE', MJOptionPane.ERROR_MESSAGE);
        return;
    end
    ...
    elseif wish == MJOptionPane.NO_OPTION
        ...
        if found
            ErrMsg = [prompt,sprintf('\n'), fclist, xlate('Check if directory is read only.')];
            edtMethod('showMessageDialog', 'com.mathworks.mwswing.MJOptionPane', ...
                frame, ErrMsg, 'GUIDE', MJOptionPane.ERROR_MESSAGE);
            return;
        end
...
% Update callback in edit callback case for suto callback generation
    ...
    if ~isempty(data)
        ...
        for i=1:length(hndls)
            ...
            if guidemfile('isAutoGeneratedCallback',hndls{i}, whichCb, source)
                ...
                message = ['The current value of %s has been manually modified.\n\n'...
                           'Do you want to replace it with the GUIDE auto-generated callback? '...
                           'Click Yes to get the auto-generated callback. '...
                           'Click No to keep the %s value as it is.'];
                if isequal(questdlg(sprintf(message, whichCb, whichCb), 'GUIDE', 'Yes', 'No', 'No'), 'Yes')
                    guidemfile('setAutoCallback',obj, whichCb);
                end
            end
        end
...
if success
    ...
    if isempty(default_name)
        ...
        % Windows
        default_name = fullfile(oldp, [oldf, '.fig']);
    end
    ...
    if ~isempty(exportmfile)
        ...
        if options.mfile
            ...
            if ~same
                ...
                if status == 0
                    error(['MATLAB:', mfilename, ':FileCopyFailed'], msg)
                end
            end
        ...
        elseif filterindex == exeindex
            ...
            if ~exist(fullfile(filepath,[filename '_files']),'dir')
                ...
                if status == 0
                    error(['MATLAB:', mfilename, ':CannotMakeDirectory'], msg)
                end
            end
...
for i = 1:limit
    ...
    if isExternalControl(obj)
        ...
        peer{i} = adpt{i};
        prox{i} = com.mathworks.toolbox.matlab.guide.palette.GObjectProxy(adpt{i},str,parentHeight);
    end
...
% This handles the menu children
if length(objects) == 1 && isa(handle(objects),'figure')
    [menus, menuparents]= getGuiMenus(objects);
end

% This handles the toolbar& its children
if length(objects) == 1 && isa(handle(objects),'figure')
...
[filename, pathname] = uigetfile(filespec, 'Open', lastdir);
...
% remove GUIDE figure flag at last right before save
...
%         if isa(handle(handles{k}),'figure')
...
out = [
    ...
    '% always return true for exist, this keeps the compiler happy but disables',NL,...
...
if ~isvarname(newTag)
    ...
    % show warning dialog if more than one component is usign the same tag
...
for i=1:length(eventlist)
    if ~isprop(peer, eventlist{i})
        ...
        p= schema.prop(peer, eventlist{i}, 'Callback');
...
peer = uicontrol('parent', fig, 'position', position, 'style', 'text');
%uistack(peer,'bottom');
...
% ****************************************************************************
function out = showErrorDialog(message)
out=[];
...
if ~isempty(e)
    ...
    for i=1:length(e.stack)
        ...
        stackinfo = sprintf('%s\nError in ==> ', stackinfo);
        file = '';
        if isfield(step,'file')
            if isempty(gotofile)
                gotofile = step.file;
            end
            [p,file] = fileparts(step.file);
            stackinfo = sprintf('%s%s', stackinfo, file);
        end
        if isfield(step,'name') &&  ~strcmp(step.name, file)
            stackinfo = sprintf('%s>%s', stackinfo, step.name);
        end
        if isfield(step,'line')
            if gotoline <0
                gotoline = step.line;
            end
            stackinfo = sprintf('%s at %i', stackinfo, step.line);
        end
    end
end
...
com.mathworks.mlservices.MLEditorServices.openDocumentToLine(file,line,1,1);
...
delete(fig);
    ...
    %Swallow any error in preloading
end
...
if isempty(f)
    ErrMsg = sprintf('File: %s\nis not a valid GUIDE file name. It must have at least one character.', filepath);
elseif ~isvarname(f)
    ErrMsg = sprintf (['File: %s\nis not a valid GUIDE file name.\n\n',...
        'The file name should be a valid MATLAB function name. A valid \n', ...
        'function name is a character string of letters, digits and \n', ...
        'underscores, with length <= %s and the first character a letter.'], filepath, num2str(namelengthmax));
end
if (~isempty(expectedExtension))
    ...
    exterror = false;
    if strcmpi(expectedExtension,'.fig')
    ...
    if exterror
       ErrMsg = sprintf('File: %s\nFile name must end with %s', [f e], expectedExtension);
    end
end

if ~fromSaveAs
   if ~isempty(ErrMsg)
      ErrMsg = sprintf('%s\n\nPlease change the "Filename" property in the inspector to a valid value, or use the Save As option.', ErrMsg);
   end
end
