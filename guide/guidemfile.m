...
%   $Revision: 1.1.4.17.2.4 $ $Date: 2007/10/15 13:57:19 $
...
% 'tempalte' field of GUI options. For existing GUI, it is saved in the
% 'lastFilename' field.
...
filter.sortByType=1;
all_h = handle(findAllChildFromHere(fig, filter));
...
if ~isAutoCallback(obj, whichCb)
    ...
    if ~isempty(mfunc) && ~isempty(subfunc)
        ...
        if strfind(strtrim(currentCb),f)==1 
            lineno = getSubfunctionLineNumber(mfunc,subfunc);
            if lineno
                com.mathworks.mlservices.MLEditorServices.openDocumentToLine(mfunc, lineno, 1, 1);
            else
                ...
                for j=1:length(callbacks)
                    ...
                    if isfield(toolinfo.properties, callbacks{j})
                        if isAutoCallback(children(i), callbacks{j}, DEFAULTCALLBACK)
                            % restore to the ppredefined callback available
...
if ~ischar(value)
   try
       ...
       if isa(handle(h),'uitools.uibuttongroup') && isequal(callback,'SelectionChangeFcn')
...
if isAutoGeneratedCallbackString(h,callback, value)
    ...
    if isa(h,'uitools.uibuttongroup') && isequal(callback,'SelectionChangeFcn')
        % due to limitation of uibuttongroup, we provide the selection
        % object as hObject in old GUIs. This is needed for
        % compatibility
        value = strrep(value, 'hObject', 'get(hObject,''SelectedObject'')');
    end
    value = anonymousFunction(['@(hObject,eventdata)' value]);              
end
...
% retrieving the abailable event data of HG callbacks, if any
