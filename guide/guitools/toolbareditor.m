...
% TOOLBAREDITOR GUI
%       TOOLBAREDITOR is a Graphical User Interfaace that allows GUI
...
hEditorFigure = figure(...
    ...
    'Visible','off',...
    'Renderer','zbuffer',...    % uicontrol selected property does not work on MAC with paiter
    'numbertitle','off', ...
...
mPropertyButtonStyle = struct(...
    ...
    'verticalOffset', -5);
if ismac
    hPropertyTabPanel = uipanel(...
        'Parent',hEditorFigure,...
        'Units','pixels',...
        'BorderType','none',...
        'position',[280,70,mFigurePosition(3)-280-20,360],...
        'BackgroundColor',get(0,'defaultuicontrolbackgroundcolor'));
    pos = getpixelposition(hPropertyTabPanel);
    hDummyPanel = uipanel(...
        ...
        'BorderType','none',...
        'BackgroundColor',get(0,'defaultuicontrolbackgroundcolor'));
    hToolbuttonPropertyTabButton = uicontrol(...
        'Parent',hDummyPanel,...
        'Units','pixels',...
        'position',[0,pos(4)-20,110,20],...
        'callback',@updateToolPropPanel,...
        'String',getUserString('ToolPropertyPanelTitle'));
    hToolbarPropertyTabButton = uicontrol(...
        'Parent',hDummyPanel,...
        'Units','pixels',...
        'position',[109,pos(4)-20,110,20],...
        'callback',@updateToolbarPropPanel,...
        'String',getUserString('ToolbarPropertyPanelTitle'));
    uicontrol(...
        'Parent',hDummyPanel,...
        'Units','pixels',...
        'position',[0,pos(4)-20,pos(3),1],...
        'ForegroundColor',get(0,'defaultuipanelshadowcolor'),...
        'style','Frame');
    pos = getpixelposition(hPropertyTabPanel);
    hToolbuttonPropertyTab = uipanel(...
        'Parent',hDummyPanel,...
        'Units','pixels',...
        'Background','red',...
        'Position',[0 0 pos(3) pos(4)-19],...
        'BackgroundColor',get(0,'defaultuicontrolbackgroundcolor'));
    hToolbarPropertyTab = uipanel(...
        'Parent',hDummyPanel,...
        'Units','pixels',...
        'Background','blue',...
        'Position',[0 0 pos(3) pos(4)-19],...
    ...
    hPropertyTabPanel = uitabgroup('v0', ...
        'Parent',hEditorFigure,...
        'Units','pixels',...
        'position',[280,70,mFigurePosition(3)-280-20,360],...
        'SelectionChangeFcn',@changePropPanel,...
        'BackgroundColor',get(0,'defaultuicontrolbackgroundcolor'),...
        'ButtonDownFcn',@clickInPropPanel,...
        'Margin',1);
    hToolbuttonPropertyTab = uitab('v0', ...
        'Parent',hPropertyTabPanel,...
        'Title', getUserString('ToolPropertyPanelTitle'));   % UITAB to contain the tool properties/menu
    %adding border to the tab panel
    uipanel(...
        'parent',hToolbuttonPropertyTab);
    hToolbarPropertyTab = uitab('v0', ...
        'Parent',hPropertyTabPanel,...
...
% debug = uicontrol('parent',hEditorFigure,'style','pushbutton','position',[410 560 40 20],'String','Show','Callback',@debuggingFunction);%, 'visi','off');

% Send proper output
if isGUIDETool
    mOutputArgs{1} = guide2tool;
else
    mOutputArgs{1} = i18ntool;
end
if nargout>0
    ...
    function changePropPanel(hObject, eve)%#ok MLint
        if get(hObject,'SelectedIndex') == 1
            % tool
    ...
    % Keep the properties active if clicked in properties panel.
    function clickInPropPanel %#ok MLint
        % disp('clicked Properties Panel')
        updateToolPropPanel();
    end
    ...
    function BUTTON = createPreviewTool(hObject,STYLE, ICON, TOOLTIP, ORDER, hiddenTool) %#ok Mlint
        ...
        BUTTON.handle = uicontrol(...
            ...
            'BackgroundColor', getUIPlatformDefaultColor(),...
            'Tooltip',TOOLTIP,...
            'enable','inactive',...
            'tag','PreviewTool',...
        ...
        function listenToPropertyChange(tool, toolpreview)
            ...
            if ~isGUIDETool
                ...
                listeners{length(props)} = [];
                for i=1:length(props)
                    listeners{i} = handle.listener(obj, findprop(obj, props{i}), 'PropertyPostSet',{@propertyChanged,tool, toolpreview});
                end
                setappdata(toolpreview, 'listeners', listeners);
            end

            function propertyChanged(src, eve, hidden, preview) %#ok MLINT
                updateWhenPropertyChange(eve.AffectedObject, preview);
            end
    ...
    function updateToolPropPanel(varargin)
        ...
        if ~isempty(SEL)                
            ...
            for i=1:length(hToolPropertyControls)
                [x,y] = ind2sub(size(propDefinition),find(ismember(propDefinition,get(hToolPropertyControls(i,1), 'string'))));
                if ~isempty(x(y==2))
                    x= x(y==2);
        ...
        function openIconEditor(hObject,eventdata) %#ok Mlint
            SEL = mFinalToolbarCurrentToolIndex;
            CDAT = get(hFinalToolbarTools(SEL),'CData');
            p =fileparts(mfilename('fullpath'));
            oldpath = addpath(p);
            newCDAT = iconeditor('icon',CDAT);
            path(oldpath);
    ...
    function selectPropertyPage(which)        
        if ismac
            if strcmpi(which, 'Toolbar')
                set(hToolbuttonPropertyTab,'Visible','off');
                set(hToolbarPropertyTab,'Visible','on');
                pos = getpixelposition(hToolbuttonPropertyTabButton);
                setpixelposition(hToolbuttonPropertyTabButton,[pos(1) pos(2) pos(3) 20]);
                pos = getpixelposition(hToolbarPropertyTabButton);
                setpixelposition(hToolbarPropertyTabButton,[pos(1) pos(2) pos(3) 22]);
                children = get(hDummyPanel,'children');
                children(find(children==hToolbuttonPropertyTabButton))=[];
                children(end+1)= hToolbuttonPropertyTabButton;
%                set(hDummyPanel,'Children',children);
            else
                set(hToolbuttonPropertyTab,'Visible','on');
                set(hToolbarPropertyTab,'Visible','off');
                pos = getpixelposition(hToolbuttonPropertyTabButton);
                setpixelposition(hToolbuttonPropertyTabButton,[pos(1) pos(2) pos(3) 22]);
                pos = getpixelposition(hToolbarPropertyTabButton);
                setpixelposition(hToolbarPropertyTabButton,[pos(1) pos(2) pos(3) 20]);
                children = get(hDummyPanel,'children');
                children(find(children==hToolbarPropertyTabButton))=[];
                children(end+1)= hToolbarPropertyTabButton;
%               set(hDummyPanel,'Children',children);
            end
            ...
            set(hPropertyTabPanel,'SelectedIndex',index);
        end
    end
    ...
    function createToolPalette
        ...
        for outer_i = 1:length(toolInfo)
            ...
            if isfield(toolInfo(outer_i), 'readyForGUIDE') && toolInfo(outer_i).readyForGUIDE
                % create a temporary tool to get its icon and other propertyies
                % column one
                ...
                hPaletteButtons(counter+2+1) = uicontrol(...
                    ...
                    'ButtonDownFcn',@paletteButtonDrag,...
                    'KeyPressFcn',@figureKeyPress,...
                    'String',getUserString(toolInfo(outer_i).name),...
...
%------------------------------------------------------------------
        ...
        function removeSelectedPreviewTool(hObject,eventdata) %#ok MLINT
            if(isempty(mPreviewSelectionIndex))
                ...
                if mToolbarToolCounter == 0
                    ...
                    set(mPreviewTools(mPreviewSelectionIndex).handle,'selected','on');
                    if isGUIDETool
    ...
    function toolapi = i18ntool
        toolapi = struct(...
            'getStringTable',@populateUserStringTable);
    end
            ...
            if ~isempty(order)
                if ~isempty(mFinalToolbarCurrentToolIndex) % && mFinalToolbarCurrentToolIndex <= length(mFinalToolbarCurrentToolIndex)
                    for ind = 1:length(mPreviewTools)
                        set(mPreviewTools(ind).handle,'Selected','off');
                    end
                end
                mFinalToolbarCurrentToolIndex = order;
                if ~isempty(order)
                    set(mPreviewTools(order).handle,'Selected','on');
                    mPreviewSelectionIndex = order;
                ...
                if double(tool) == double(hFinalToolbar)
                    if ~isempty(mPreviewSelectionIndex)
                        set(mPreviewTools(mPreviewSelectionIndex).handle,'Selected','off');
                    end
    ...
    function updateWhenPropertyChange(varargin)
        ...
        if ~isa(tool, 'uitoolbar');
            index = find(ismember(hFinalToolbarTools, tool));
            if ~isempty(index)
    ...
    function userString = populateUserStringTable()
        userString = struct(...
        'FigureName', 'Toolbar Editor', ...
        'ToolbarLayoutTitle','Toolbar Layout',...
        'ToolbarLayoutHelp','Add tools here to create a toolbar. Drag and drop from the palette or press the Add button',...
        'PalettePushTool', 'Push Tool',...
        'PaletteToggleTool','Toggle Tool',...
        'ToolPropertyPanelTitle','Tool Properties',...
        'ToolbarPropertyPanelTitle','Toolbar Properties',...
        'ToolbatDeletionConfirmation','Deleting this toolbar will also delete all the tools it has. \n\nDo you want to continue?',...
        'ToolPalettePanelTitle','Tool Palette',...
        'CustomToolSectionTitle','Custom Tools',...
        'PredefinedToolSectionTitle','Predefined Tools',...
        'ActionHelp', 'Help',...
        'ActionHelpTooltip', '',...
        'ActionOKTooltip', '',...
        'ActionAdd', 'Add',...
        'ActionAddTooltip', 'Add selected palette item to toolbar',...
        'ActionDelete', 'Delete',...
        'ActionDeleteTooltip', 'Remove selected toolbar item',...
        'ActionSendtoFirst', 'Send to First',...
        'ActionSendtoFirstTooltip','Move selected toolbar item to the beginning',...
        'ActionSendtoLast', 'Send to Last',...
        'ActionSendtoLastTooltip','Move selected toolbar item to the end',...
        'ActionMovetotheRight','Move to the Right',...
        'ActionMovetotheRightTooltip','Move selected toolbar item to the right',...
        'ActionMovetotheLeft','Move to the Left',...
        'ActionMovetotheLeftTooltip','Move selected toolbar item to the left',...
        'ActionDeleteToolbar','Delete Toolbar',...
        'ActionDeleteToolbarTooltip','Delete this toolbar and all its tools',...
        'ActionAddToolbar','Add Toolbar',...
        'ActionAddToolbarTooltip','Add a new empty toolbar',...
        'ActionShiftPreviewToolRight','Shift Preview Tool Right',...
        'ActionShiftPreviewToolRightTooltip','Shift the preview tools starting from given position right',...
        'ActionShiftPreviewToolLeft','Shift Preview Tool Left',...
        'ActionShiftPreviewToolLeftTooltip','Shift the preview tools starting from given position left',...
        'ActionEditIcon','Edit Icon',...
        'ActionEditIconTooltip','Change the icon of the selected toolbar item ',...
        'ActionEditTooltip','Edit Tooltip',...
        'ActionEditTooltipTooltip','Change the tooltip of the selected toolbar item ',...
        'ActionShowSeparator','Show Separator',...
        'ActionShowSeparatorTooltip','Turn the separator on/off for the selected toolbar item ',...
        'ActionMorePropertiesTooltip','Edit all properties in Inspector',...
        'ActionMoreProperties','More Property ...',...
        'EditCallback','View',...
        'RestoreDefaults','Restore Defaults',...
        'RestoreDefaultsTooltip','Go back to the default behavior of this predefined tool',...
        'PropertyCData','CData (icon):',...
        'PropertyEnablethistool','Enable this tool',...
        'PropertyOffCallback','Off Callback:',...
        'PropertyOnCallback','On Callback:',...
        'PropertySeparatoronleftside','Separator on left side',...
        'PropertyTag','Tag:',...
        'PropertyTooltipString','Tooltip String:',...    
        'PropertyClickedCallback','Clicked Callback',...
        'PropertyMaketoolbarvisible','Make toolbar visible',...
        'PropertyEditIcon','Edit ...',...
        'RestoreDefaultConfirmation', 'The selected toolbar tool was created from a predefined tool on the palette. Restoring its default behavior will remove all the changes you may have made.\n\nDo you want to continue?' ...
        );

       toolinfo = uitoolfactory('getinfo');
        for i=1:length(toolinfo)
            if toolinfo(i).readyForGUIDE
                userString.(toolinfo(i).name) = toolinfo(i).label;
            end
        end
    end
